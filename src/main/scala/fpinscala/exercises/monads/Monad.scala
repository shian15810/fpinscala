...
trait Monad[F[_]] extends Functor[F]:
  ...
  extension [A](fa: F[A]) def flatMapViaCompose[B](f: A => F[B]): F[B] = ???

  def filterM[A](as: List[A])(f: A => F[Boolean]): F[List[A]] = ???

  extension [A](ffa: F[F[A]]) def join: F[A] = ???

  extension [A](fa: F[A]) def flatMapViaJoinAndMap[B](f: A => F[B]): F[B] = ???
...
object Monad:
  ...
  def parserMonad[P[+_]](p: Parsers[P]): Monad[P] = new:
    def unit[A](a: => A)                                                = ???
    extension [A](fa: P[A]) override def flatMap[B](f: A => P[B]): P[B] = ???
  ...
  given listMonad: Monad[List] with
    def unit[A](a: => A)                                                = ???
    extension [A](fa: List[A]) override def flatMap[B](f: A => List[B]) = ???
...
object Id:
  given idMonad: Monad[Id] with
    def unit[A](a: => A)                                            = ???
    extension [A](fa: Id[A]) override def flatMap[B](f: A => Id[B]) = ???
