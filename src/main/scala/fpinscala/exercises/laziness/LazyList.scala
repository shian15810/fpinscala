...
enum LazyList[+A]:
  ...
  def map[B](f: (A => B)): LazyList[B] = foldRight(empty)((a, b) => cons(f(a), b))
  ...
  def startsWith[B](s: LazyList[B]): Boolean = zipAll(s)
    .takeWhile(a => a._2.isDefined).forAll((a1, a2) => a1 == a2)
...
object LazyList:
  ...
  def fib(n: Int) =
    def loop(m: Int, curr: Int, next: Int): Int =
      if m == n then curr else loop(m + 1, next, curr + next)
    loop(0, 0, 1)
