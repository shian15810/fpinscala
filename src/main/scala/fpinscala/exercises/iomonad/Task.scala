...
object Task:
  ...
  extension [A](self: Task[A])
    def flatMap[B](f: A => Task[B]): Task[B] = IO.monad.flatMap(self):
    ...
    def attempt: Task[Either[Throwable, A]] = IO.monad.map(self):
      case Left(e)  => Right(Left(e))
      case Right(a) => Right(Right(a))

    def handle[B >: A](f: PartialFunction[Throwable, B]): Task[B] = attempt
      .flatMap:
        case Left(e)  => f.lift(e).map(Task.now).getOrElse(Task.fail(e))
        case Right(a) => Task.now(a)

    def or[B >: A](t2: Task[B]): Task[B] = IO.monad.flatMap(self):
      case Left(e) => t2
      case a       => IO(a)
  ...
  def fork[A](a: => Task[A]): Task[A] = IO.par(Par.lazyUnit(())).flatMap(_ => a)
