...
enum Validated[+E, +A]:
  ...
  def map2[EE >: E, B, C](b: Validated[EE, B])(
      f: (A, B) => C): Validated[EE, C] = (this, b) match
...
object Validated:
  ...
  def traverse[E, A, B](
      as: List[A],
      f: A => Validated[E, B]): Validated[E, List[B]] = as
    .foldRight(Valid(Nil): Validated[E, List[B]])(
...
object MoreGeneralVersionOfValidated:
  ...
  enum Validated[+E, +A]:
    ...
    def map2[EE >: E, B, C](b: Validated[EE, B])(f: (A, B) => C)(
        combineErrors: (EE, EE) => EE): Validated[EE, C] = (this, b) match
      case (Valid(aa), Valid(bb))     => Valid(f(aa, bb))
  ...
  object Validated:
    ...
    def traverse[E, A, B](
        as: List[A],
        f: A => Validated[E, B],
        combineErrors: (E, E) => E): Validated[E, List[B]] = as
      .foldRight(Valid(Nil): Validated[E, List[B]])(
        (a, acc) => f(a).map2(acc)(_ :: _)(combineErrors))

    def sequence[E, A](
        vs: List[Validated[E, A]],
        combineErrors: (E, E) => E): Validated[E, List[A]] =
      traverse(vs, identity, combineErrors)
  end Validated
end MoreGeneralVersionOfValidated
