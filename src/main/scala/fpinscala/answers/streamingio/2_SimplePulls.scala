...
object SimplePulls:
  ...
  enum Pull[+O, +R]:
    ...
    @annotation.tailrec
    final def fold[A](init: A)(f: (A, O) => A): (R, A) = step match
      case Left(r)         => (r, init)
      case Right((hd, tl)) => tl.fold(f(init, hd))(f)

    def toList: List[O] = fold(List.newBuilder[O])((bldr, o) => bldr += o)(1)
      .result
    ...
    def uncons: Pull[Nothing, Either[R, (O, Pull[O, R])]] = Pull.done >>
      Result(step)
    ...
    def dropWhile(f: O => Boolean): Pull[Nothing, Pull[O, R]] = uncons.flatMap:
      case Left(r) => Result(Result(r))
      case Right((hd, tl)) =>
    ...
    def tally[O2 >: O](using m: Monoid[O2]): Pull[O2, R] =
      def go(total: O2, p: Pull[O, R]): Pull[O2, R] = p.uncons.flatMap:
        case Left(r) => Result(r)
        case Right((hd, tl)) =>
    ...
    def countViaMapAccumulate: Pull[Int, R] = Output(0) >>
      mapAccumulate(0)((s, o) => (s + 1, s + 1)).map(_(1))
  ...
  object Pull:
    ...
    def continually[O](o: O): Pull[O, Nothing] = Output(o) >> continually(o)

    def continuallyViaRepeat[O](o: O): Pull[O, Nothing] = Output(o).repeat

    def iterate[O](initial: O)(f: O => O): Pull[O, Nothing] = Output(initial) >>
      iterate(f(initial))(f)
    ...
    extension [R](self: Pull[Int, R])
      def slidingMean(n: Int): Pull[Double, R] =
        def go(
            ...
            p: Pull[Int, R]): Pull[Double, R] = p.uncons.flatMap:
          case Left(r) => Result(r)
          case Right((hd, tl)) =>
            val newWindow =
      ...
      def slidingMeanViaMapAccumulate(n: Int): Pull[Double, R] = self
    ...
    extension [O](self: Pull[O, Unit])
      def flatMapOutput[O2](f: O => Pull[O2, Unit]): Pull[O2, Unit] = self
        .uncons.flatMap:
          case Left(())        => Result(())
    ...
    val outputMonad: Monad[[x] =>> Pull[x, Unit]] = new:
      ...
      extension [A](pa: Pull[A, Unit])
        def flatMap[B](f: A => Pull[B, Unit]): Pull[B, Unit] = pa
          .flatMapOutput(f)
  ...
  object Stream:
    def apply[O](os: O*): Stream[O] = Pull.fromList(os.toList).toStream
    extension [O](self: Stream[O])
      def toPull: Pull[O, Unit] = self

      def fold[A](init: A)(f: (A, O) => A): A = self.fold(init)(f)(1)

      def toList: List[O] = self.toList

      def take(n: Int): Stream[O] = self.take(n).void

      def filter(p: O => Boolean): Stream[O] = self.filter(p).toStream
    ...
    given Monad[Stream] with
      ...
      extension [A](sa: Stream[A])
        def flatMap[B](f: A => Stream[B]): Stream[B] = sa.flatMapOutput(f)
  end Stream
...
object SimplePullExamples:
  ...
  val lowerCase: Pipe[String, String] = _.map(_.toLowerCase)
  ...
  def takeThrough[I](f: I => Boolean): Pipe[I, I] = src =>
    src.toPull.takeWhile(f).flatMap(remainder => remainder.take(1)).void
      .toStream

  def dropWhile[I](f: I => Boolean): Pipe[I, I] =
    src => src.toPull.dropWhile(f).flatMap(remainder => remainder).void.toStream

  def existsHalting[I](f: I => Boolean): Pipe[I, Boolean] = exists(f)
    .andThen(takeThrough(!_)).andThen(dropWhile(!_))

  def last[I](init: I): Pipe[I, I] =
    def go(value: I, p: Pull[I, Unit]): Pull[I, Unit] = p.uncons.flatMap:
      case Left(_)         => Pull.Output(value)
      case Right((hd, tl)) => go(hd, tl)
    src => go(init, src.toPull).toStream

  def existsHalting2[I](f: I => Boolean): Pipe[I, Boolean] = exists(f)
    .andThen(takeThrough(!_)).andThen(last(false))

  def countGt40K[I]: Pipe[I, Boolean] = count.andThen(existsHalting(_ > 40000))

  def fromIterator[O](itr: Iterator[O]): Stream[O] = Pull.unfold(itr)(
    itr => if itr.hasNext then Right((itr.next(), itr)) else Left(itr)).void
    .toStream

  def processFile[A](file: java.io.File, p: Pipe[String, A])(using
      m: Monoid[A]): IO[A] = IO:
  ...
  def trimmed: Pipe[String, String] = src => src.map(_.trim)

  def nonComment: Pipe[String, String] = src => src.filter(_.charAt(0) != '#')

  def asDouble: Pipe[String, Double] = src =>
    src.flatMap: s =>
      s.toDoubleOption match
        case Some(d) => Stream(d)
        case None    => Stream()

  def convertToCelsius: Pipe[Double, Double] = src => src.map(toCelsius)

  val conversion: Pipe[String, Double] = trimmed.andThen(nonEmpty)
  ...
  def convert(inputFile: String, outputFile: String): IO[Unit] = IO:
    ...
    try
      ...
      try fromIterator(source.getLines).pipe(conversion).fold(()): (_, a) =>
          writer.write(a.toString)
          writer.newLine()
      finally writer.close()
    finally source.close()
end SimplePullExamples
