...
object EffectfulPulls:
  ...
  enum Pull[+F[_], +O, +R]:
    ...
    def step[F2[x] >: F[x], O2 >: O, R2 >: R](using
        F: Monad[F2]): F2[Either[R2, (O2, Pull[F2, O2, R2])]] = this match
    ...
    def fold[F2[x] >: F[x], R2 >: R, A](init: A)(f: (A, O) => A)(using
        F: Monad[F2]): F2[(R2, A)] = step.flatMap:
  ...
  object Stream:
    ...
    extension [F[_], O](self: Stream[F, O])
      ...
      def fold[A](init: A)(f: (A, O) => A)(using Monad[F]): F[A] = self
        .fold(init)(f).map(_(1))

      def toList(using Monad[F]): F[List[O]] = self.toList

      def run(using Monad[F]): F[Unit] = fold(())((_, _) => ()).map(_(1))
      ...
      def flatMap[O2](f: O => Stream[F, O2]): Stream[F, O2] = self
        .flatMapOutput(f)

      def mapEval[O2](f: O => F[O2]): Stream[F, O2] = Stream
        .flatMap(self)(o => Stream.eval(f(o)))
    end extension

    extension [O](self: Stream[Nothing, O])
      def fold[A](init: A)(f: (A, O) => A): A = self
        .fold(init)(f)(using Monad.tailrecMonad).result(1)

      def toList: List[O] = self.toList(using Monad.tailrecMonad).result
