...
object ErrorHandling:
  ...
  enum Pull[+F[_], +O, +R]:
    ...
    def step[F2[x] >: F[x], O2 >: O, R2 >: R](using
        F: MonadThrow[F2]): F2[Either[R2, (O2, Pull[F2, O2, R2])]] = this match
      case Result(r)      => F.unit(Left(r))
      case Output(o)      => F.unit(Right(o, Pull.done))
      case Eval(action)   => action.map(Left(_))
      case Uncons(source) => source.step.map(s => Left(s.asInstanceOf[R2]))
      case Handle(source, f) => source match
          case Handle(s2, g) => s2
              .handleErrorWith(x => g(x).handleErrorWith(y => f(y))).step
          case other => other.step.map:
    ...
    def fold[F2[x] >: F[x], R2 >: R, A](init: A)(f: (A, O) => A)(using
        F: MonadThrow[F2]): F2[(R2, A)] = step.flatMap:
    ...
    def handleErrorWith[F2[x] >: F[x], O2 >: O, R2 >: R](
        handler: Throwable => Pull[F2, O2, R2]): Pull[F2, O2, R2] = Pull
      .Handle(this, handler)
  end Pull
  ...
  object Pull:
    ...
    extension [F[_], O](self: Pull[F, O, Unit])
      def flatMapOutput[O2](f: O => Pull[F, O2, Unit]): Pull[F, O2, Unit] = self
        .uncons.flatMap:
          case Left(())        => Result(())
          case Right((hd, tl)) => f(hd) >> tl.flatMapOutput(f)
...
object ErrorHandlingExample:
  ...
  def acquire(path: String): Task[Source] = Task(Source.fromFile(path))

  def use(source: Source): Stream[Task, Unit] = Stream.eval(Task(source.getLines))
    .flatMap(itr => Stream.fromIterator(itr)).mapEval(line => Task(println(line)))

  def release(source: Source): Task[Unit] = Task(source.close())

  val prg: Stream[Task, Unit] = Stream.eval(acquire("build.sbt"))
    .flatMap(resource => use(resource).onComplete(Stream.eval(release(resource))))
