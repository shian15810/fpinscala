...
object Resources:
  ...
  object Ref:
    def apply[F[_], A](initial: A)(using F: Monad[F]): Ref[F, A] = new Ref(
  ...
  final class Scope[F[_]](
      ...
      val id: Id,
      state: Ref[F, Scope.State[F]])(using F: MonadThrow[F]):
    import Scope.State

    def open(finalizer: F[Unit]): F[Scope[F]] = state.modify:
      case State.Open(myFinalizer, subscopes) =>
        val sub =
          new Scope(Some(this), new Id, Ref(State.Open(finalizer, Vector.empty)))
        State.Open(myFinalizer, subscopes :+ sub) -> F.unit(sub)
      case State.Closed() =>
        val next = parent match
          case None => F
              .raiseError(new RuntimeException("root scope already closed"))
    ...
    def close: F[Unit] = state.modify:
      case State.Open(finalizer, subscopes) =>
        val finalizers =
          (subscopes.reverseIterator.map(_.close) ++ Iterator(finalizer)).toList
        def go(rem: List[F[Unit]], error: Option[Throwable]): F[Unit] =
          rem match
            case Nil => error match
                case None    => F.unit(())
                case Some(t) => F.raiseError(t)
            case hd :: tl => hd.attempt
                .flatMap(res => go(tl, error.orElse(res.toEither.swap.toOption)))
        State.Closed() -> go(finalizers, None)
      case State.Closed() => State.Closed() -> F.unit(())
    .flatten

    def findScope(target: Id): F[Option[Scope[F]]] = findThisOrSubScope(target)
      .flatMap:
      ...
      if id == target then F.unit(Some(this))
        ...
        state.get.flatMap:
          case State.Open(_, subscopes) =>
            def go(rem: List[Scope[F]]): F[Option[Scope[F]]] = rem match
  ...
  enum Pull[+F[_], +O, +R]:
    ...
    case OpenScope(source: Pull[F, O, R], finalizer: Option[F[Unit]])
      extends Pull[F, O, R]
    case WithScope(source: Pull[F, O, R], scopeId: Id, returnScope: Id)
      extends Pull[F, O, R]

    def step[F2[x] >: F[x], O2 >: O, R2 >: R](scope: Scope[F2])(using
        F: MonadThrow[F2]): F2[StepResult[F2, O2, R2]] =
      ...
      this match
        ...
        case Uncons(source) => source.step(scope)
            .map(s => Done(scope, s.toUnconsResult.asInstanceOf[R2]))
        case Handle(source, f) => source match
            case Handle(s2, g) => s2
                .handleErrorWith(x => g(x).handleErrorWith(y => f(y)))
                .step(scope)
            case other => other.step(scope).map:
        ...
        case FlatMap(source, f) => source match
            case FlatMap(s2, g) => s2.flatMap(x => g(x).flatMap(y => f(y)))
                .step(scope)
            case other => other.step(scope).flatMap:
                case Done(scope, r)     => f(r).step(scope)
                case Out(scope, hd, tl) => F.unit(Out(scope, hd, tl.flatMap(f)))
        case FlatMapOutput(source, f) => source.step(scope).flatMap:
            case Done(scope, r) => F.unit(Done(scope, r))
            case Out(scope, hd, tl) => f(hd).flatMap(_ => tl.flatMapOutput(f))
                .step(scope)
        case OpenScope(source, finalizer) => scope
            .open(finalizer.getOrElse(F.unit(()))).flatMap(
              subscope => WithScope(source, subscope.id, scope.id).step(subscope))
        case WithScope(source, scopeId, returnScopeId) => scope
            .findScope(scopeId).map(_.map(_ -> true).getOrElse(scope -> false))
            .flatMap:
              case (newScope, closeAfterUse) => source.step(newScope).attempt
                  .flatMap:
                    case Success(Out(scope, hd, tl)) =>
                      F.unit(Out(scope, hd, WithScope(tl, scopeId, returnScopeId)))
                    case Success(Done(outScope, r)) => scope
                        .findScope(returnScopeId).map(_.getOrElse(outScope))
                        .flatMap: nextScope =>
                          scope.close.as(Done(nextScope, r))
                    case Failure(t) => scope.close.flatMap(_ => F.raiseError(t))
      end match
    end step

    def fold[F2[x] >: F[x], R2 >: R, A](init: A)(f: (A, O) => A)(using
        F: MonadThrow[F2]): F2[(R2, A)] =
      val scope = Scope.root[F2]
      def go(scope: Scope[F2], p: Pull[F2, O, R2], acc: A): F2[(R2, A)] = p
        .step(scope).flatMap:
          case StepResult.Done(_, r)            => F.unit((r, init))
          case StepResult.Out(newScope, hd, tl) => go(newScope, tl, f(init, hd))
      go(scope, this, init).attempt
        .flatMap(res => scope.close.flatMap(_ => res.fold(F.raiseError, F.unit)))
    ...
    def flatMap[F2[x] >: F[x], O2 >: O, R2](
        f: R => Pull[F2, O2, R2]): Pull[F2, O2, R2] = FlatMap(this, f)

    def >>[F2[x] >: F[x], O2 >: O, R2](
        next: => Pull[F2, O2, R2]): Pull[F2, O2, R2] = flatMap(_ => next)

    def map[R2](f: R => R2): Pull[F, O, R2] = this.flatMap(r => Result(f(r)))
    ...
    def takeWhile(f: O => Boolean): Pull[F, O, Pull[F, O, R]] = uncons.flatMap:
      ...
      case Right((hd, tl)) =>
        if f(hd) then Output(hd) >> tl.takeWhile(f) else Result(Output(hd) >> tl)

    def dropWhile(f: O => Boolean): Pull[F, Nothing, Pull[F, O, R]] = uncons
      .flatMap:
        case Left(r) => Result(Result(r))
        case Right((hd, tl)) =>
          if f(hd) then tl.dropWhile(f) else Result(Output(hd) >> tl)
    ...
    def filter(p: O => Boolean): Pull[F, O, R] = uncons.flatMap:
      case Left(r) => Result(r)
      case Right((hd, tl)) => (if p(hd) then Output(hd) else Pull.done) >>
          tl.filter(p)

    def count: Pull[F, Int, R] =
      def go(total: Int, p: Pull[F, O, R]): Pull[F, Int, R] = p.uncons.flatMap:
        case Left(r) => Result(r)
        case Right((_, tl)) =>
    ...
    def tally[O2 >: O](using m: Monoid[O2]): Pull[F, O2, R] =
      def go(total: O2, p: Pull[F, O, R]): Pull[F, O2, R] = p.uncons.flatMap:
        case Left(r) => Result(r)
        case Right((hd, tl)) =>
    ...
    def mapAccumulate[S, O2](init: S)(
        f: (S, O) => (S, O2)): Pull[F, O2, (S, R)] = uncons.flatMap:
  ...
  object Pull:
    ...
    def unfoldEval[F[_], O, R](init: R)(
        f: R => F[Either[R, (O, R)]]): Pull[F, O, R] = Pull.Eval(f(init))
      .flatMap:
        case Left(r)        => Result(r)
    ...
    extension [F[_], R](self: Pull[F, Int, R])
      def slidingMean(n: Int): Pull[F, Double, R] =
        def go(
            ...
            p: Pull[F, Int, R]): Pull[F, Double, R] = p.uncons.flatMap:
          case Left(r) => Result(r)
          case Right((hd, tl)) =>
            val newWindow =
    ...
    given [F[_], O]: Monad[[x] =>> Pull[F, O, x]] with
      ...
      extension [A](pa: Pull[F, O, A])
        def flatMap[B](f: A => Pull[F, O, B]): Pull[F, O, B] = pa.flatMap(f)
  ...
  object Stream:
    ...
    def apply[O](os: O*): Stream[Nothing1, O] = fromList(os.toList)

    def fromList[O](os: List[O]): Stream[Nothing1, O] = os match
      case Nil      => Pull.done
      case hd :: tl => Pull.Output(hd) >> fromList(tl)

    def fromLazyList[O](os: LazyList[O]): Stream[Nothing1, O] = os match
    ...
    def continually[O](o: O): Stream[Nothing1, O] = Pull.Output(o) >>
      continually(o)

    def iterate[O](initial: O)(f: O => O): Stream[Nothing1, O] = Pull
      .Output(initial) >> iterate(f(initial))(f)

    def eval[F[_], O](fo: F[O]): Stream[F, O] = Pull.Eval(fo)
      .flatMap(Pull.Output(_))

    def unfoldEval[F[_], O, R](init: R)(
        f: R => F[Option[(O, R)]]): Stream[F, O] = Pull.Eval(f(init)).flatMap:
    ...
    extension [F[_], O](self: Stream[F, O])
      ...
      def fold[A](init: A)(f: (A, O) => A)(using MonadThrow[F]): F[A] = self
        .fold(init)(f).map(_(1))

      def toList(using MonadThrow[F]): F[List[O]] = self.toList

      def run(using MonadThrow[F]): F[Unit] = fold(())((_, _) => ()).map(_(1))

      def ++(that: => Stream[F, O]): Stream[F, O] = self >> that

      def repeat: Stream[F, O] = self.repeat

      def take(n: Int): Stream[F, O] = self.take(n).void.scope

      def filter(p: O => Boolean): Stream[F, O] = self.filter(p)

      def mapEval[O2](f: O => F[O2]): Stream[F, O2] = self
        .flatMapOutput(o => Stream.eval(f(o)))
      ...
      def drain: Stream[F, Nothing] = self.flatMapOutput(o => Stream.empty)

      def scope: Stream[F, O] = Pull.OpenScope(self, None)
    end extension

    extension [O](self: Stream[Nothing, O])
      def fold[A](init: A)(f: (A, O) => A): A = (self: Stream[SyncTask, O])
        .fold(init)(f).resultOrThrow(1)

      def toList: List[O] = (self: Stream[SyncTask, O]).toList.resultOrThrow

    given [F[_]]: Monad[[x] =>> Stream[F, x]] with
      def unit[A](a: => A): Stream[F, A] = Pull.Output(a)
      extension [A](sa: Stream[F, A])
        def flatMap[B](f: A => Stream[F, B]): Stream[F, B] = sa.flatMapOutput(f)
  end Stream
...
object ResourcesExample:
  ...
  def file(path: String): Stream[Task, Source] = Stream
    .resource(Task(Source.fromFile(path)))(
      s => Task { println("CLOSING"); s.close() })

  def lines(path: String): Stream[Task, String] = file(path).flatMap(
    source => Stream.eval(Task(source.getLines)).flatMap(Stream.fromIterator))

  val printLines: Stream[Task, Unit] = lines("build.sbt")
    .mapEval(line => Task(println(line)))

  def fileWriter(path: String): Stream[Task, BufferedWriter] = Stream
    .resource(Task(Files.newBufferedWriter(Paths.get(path))))(w => Task(w.close()))

  def writeLines(path: String): Pipe[Task, String, Unit] = lines =>
    fileWriter(path).flatMap(
      writer =>
        lines.mapEval(
          line =>
            Task:
  ...
  def trimmed[F[_]]: Pipe[F, String, String] = src => src.map(_.trim)

  def nonEmpty[F[_]]: Pipe[F, String, String] = _.filter(_.nonEmpty)

  def nonComment[F[_]]: Pipe[F, String, String] =
    src => src.filter(_.charAt(0) != '#')

  def asDouble[F[_]]: Pipe[F, String, Double] = src =>
    src.flatMap: s =>
      s.toDoubleOption match
        case Some(d) => Stream(d)
        case None    => Stream()

  def convertToCelsius[F[_]]: Pipe[F, Double, Double] = src => src.map(toCelsius)

  def toString[F[_], A]: Pipe[F, A, String] = _.map(_.toString)

  val conversion: Pipe[Task, String, String] = trimmed.andThen(nonEmpty)
    .andThen(nonComment).andThen(asDouble).andThen(convertToCelsius)
    .andThen(toString)
  ...
  def convertAll(inputFile: String, outputFile: String): Task[Unit] =
    lines(inputFile).flatMap(lines).pipe(conversion).pipe(writeLines(outputFile))
      .run

  def convertMultisink(inputFile: String): Task[Unit] = lines(inputFile).flatMap(
    file => lines(file).pipe(conversion).pipe(writeLines(file + ".celsius")))
    .run
end ResourcesExample
