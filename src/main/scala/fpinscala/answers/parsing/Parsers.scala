...
trait Parsers[Parser[+_]]:
  ...
  /**
   * C/Java style floating point literals, e.g .1, -1.0, 1e9, 1E-23, etc. Result
   * is left as a string to keep full precision
   */
  ...
  def eof: Parser[String] = regex("\\z".r)
    .label("unexpected trailing characters")
  ...
  extension [A](p: Parser[A])
    ...
    def listOfN(n: Int): Parser[List[A]] =
      if n <= 0 then succeed(Nil) else p.map2(p.listOfN(n - 1))(_ :: _)

    def map[B](f: A => B): Parser[B] = p.flatMap(f.andThen(succeed))

    def map2[B, C](p2: => Parser[B])(f: (A, B) => C): Parser[C] = p.product(p2)
      .map((a, b) => f(a, b))
    ...
    def product[B](p2: => Parser[B]): Parser[(A, B)] = p
      .flatMap(a => p2.map(b => (a, b)))
     ...
     * ignored.
     */
    def sep1(separator: Parser[Any]): Parser[List[A]] = p
      .map2((separator *> p).many)(_ :: _)
     ...
     * precedence.
     */
    def opL(op: Parser[(A, A) => A]): Parser[A] = p
      .map2((op ** p).many)((h, t) => t.foldLeft(h)((a, b) => b._1(a, b._2)))
...
case class ParseError(stack: List[(Location, String)] = Nil):
   ...
   * location are combined on one line. For the bottommost error, we display the
   * full line, with a caret pointing to the column of the error. Example:
   *
  ...
  override def toString =
    if stack.isEmpty then "no error message"
      ...
      val context = collapsed.lastOption.map("\n\n" + _._1.currentLine)
        .getOrElse("") + collapsed.lastOption.map("\n" + _._1.columnCaret)
        .getOrElse("")
  ...
  def collapseStack(s: List[(Location, String)]): List[(Location, String)] = s
    .groupBy(_._1).view.mapValues(_.map(_._2).mkString("; ")).toList
    .sortBy(_._1.offset)
