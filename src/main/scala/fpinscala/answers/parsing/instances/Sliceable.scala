...
object Sliceable extends Parsers[Sliceable.Parser]:
   ...
   * means that not all the data constructors of `Result` have the same type. In
   * particular, `Slice` _refines_ the `A` type parameter to be `String`. If we
   * pattern match on a `Result` and obtain a `Slice`, we expect to be able to
   * assume that `A` was in fact `String` and use this type information
   * elsewhere.
   */
  enum Result[+A]:
  ...
  /**
   * Returns -1 if s.startsWith(s2), otherwise returns the first index where the
   * two strings differed. If s2 is longer than s1, returns s.length.
   */
  def firstNonmatchingIndex(s: String, s2: String, offset: Int): Int =
    ...
    while i + offset < s.length && i < s2.length do
      ...
      i += 1
    if s.length - offset >= s2.length then -1 else s.length - offset
  ...
  def regex(r: Regex): Parser[String] = s =>
    r.findPrefixOf(s.input) match
      ...
      case Some(m) =>
        if s.isSliced then Slice(m.length) else Success(m, m.length)

  def fail(msg: String): Parser[Nothing] = s => Failure(s.loc.toError(msg), true)

  extension [A](p: Parser[A])
    def run(s: String): Either[ParseError, A] = p(ParseState(Location(s), false))
      .extract(s)

    def or(p2: => Parser[A]): Parser[A] = s =>
      p(s) match
    ...
    def flatMap[B](f: A => Parser[B]): Parser[B] = s =>
      p(s.unslice) match
        case Success(a, n) => f(a)(s.advanceBy(n).reslice(s)).addCommit(n != 0)
            .advanceSuccess(n)
        case Slice(n) => f(s.slice(n))(s.advanceBy(n).reslice(s))
            .advanceSuccess(n)
        case f @ Failure(_, _) => f

    def attempt: Parser[A] = s => p(s).uncommit

    def slice: Parser[String] = s => p(s.copy(isSliced = true)).slice

    override def product[B](p2: => Parser[B]): Parser[(A, B)] = p.map2(p2)((_, _))
     ...
     * `String`. */
    override def map[B](f: A => B): Parser[B] = s =>
      p(s) match
        case Success(a, n)     => Success(f(a), n)
        case Slice(n)          => Success(f(s.slice(n)), n)
        case f @ Failure(_, _) => f

    override def map2[B, C](p2: => Parser[B])(f: (A, B) => C): Parser[C] = s =>
      p(s) match
        case Success(a, n) =>
    ...
    override def many: Parser[List[A]] = s =>
      var nConsumed: Int = 0
      if s.isSliced then
        def go(p: Parser[String], offset: Int): Result[String] =
          p(s.advanceBy(offset)) match
            ...
            case Success(_, _) => sys
                .error("sliced parser should not return success, only slice")
    ...
    def scope(msg: String): Parser[A] = s => p(s).mapError(_.push(s.loc, msg))

    def label(msg: String): Parser[A] = s => p(s).mapError(_.label(msg))
  end extension
end Sliceable
