...
enum LazyList[+A]:
  ...
  def exists(p: A => Boolean): Boolean = foldRight(false)((a, b) => p(a) || b) // Here `b` is the unevaluated recursive step that folds the tail of the lazy list. If `p(a)` returns `true`, `b` will never be evaluated and the computation terminates early.
  ...
  def zipWithAll[B, C](that: LazyList[B])(
      f: (Option[A], Option[B]) => C): LazyList[C] = LazyList
    .unfold((this, that)):
      case (Empty, Empty) => None
      case (Cons(h, t), Empty) =>
  ...
  def zipAllViaZipWithAll[B](
      s2: LazyList[B]): LazyList[(Option[A], Option[B])] = zipWithAll(s2)((_, _))
   ...
   * exhausted. */
  def startsWith[A](prefix: LazyList[A]): Boolean = zipAll(prefix)
    .takeWhile(_(1).isDefined).forAll((a1, a2) => a1 == a2)
  ...
  def hasSubsequence[A](s: LazyList[A]): Boolean = tails.exists(_.startsWith(s))
...
object LazyList:
  ...
  def unfoldViaFold[A, S](z: S)(f: S => Option[(A, S)]): LazyList[A] = f(z)
    .fold(empty[A])((p: (A, S)) => cons(p(0), unfold(p(1))(f)))

  def unfoldViaMap[A, S](z: S)(f: S => Option[(A, S)]): LazyList[A] = f(z)
    .map((p: (A, S)) => cons(p(0), unfold(p(1))(f))).getOrElse(empty[A])
  ...
  val fibsViaUnfold: LazyList[Int] = unfold((0, 1)):
  ...
  val onesViaUnfold: LazyList[Int] = unfold(())(_ => Some((1, ())))
end LazyList
