...
trait Traverse[F[_]] extends Functor[F], Foldable[F]:
  ...
  extension [A](fa: F[A])
    def traverse[G[_]: Applicative, B](f: A => G[B]): G[F[B]] = fa.map(f)
      .sequence
  ...
  extension [A](fa: F[A])
    def map[B](f: A => B): F[B] = fa.traverse[Id, B](f)(using Id.idMonad)

    override def foldMap[B: Monoid](f: A => B): B = fa
      .traverse[Const[B, _], Nothing](f)

    override def foldLeft[B](acc: B)(f: (B, A) => B): B = fa
      .mapAccum(acc)((a, b) => ((), f(b, a)))(1)

    override def toList: List[A] = fa
      .mapAccum(List[A]())((a, s) => ((), a :: s))(1).reverse

    def zipWithIndex_ : F[(A, Int)] = fa.traverse(
      a =>
        for
          i <- State.get[Int]
          _ <- State.set(i + 1)
    ...
    def toList_ : List[A] = fa.traverse(
      a =>
        for
        ...
        yield ()).run(Nil)(1).reverse

    def mapAccum[S, B](s: S)(f: (A, S) => (B, S)): (F[B], S) = fa.traverse(
      a =>
        for
          s1 <- State.get[S]
          (b, s2) = f(a, s1)
    ...
    def zipWithIndex: F[(A, Int)] = fa.mapAccum(0)((a, s) => ((a, s), s + 1))(0)

    def reverse: F[A] = fa
      .mapAccum(fa.toList.reverse)((_, as) => (as.head, as.tail))(0)

    def zip[B](fb: F[B]): F[(A, B)] = fa.mapAccum(fb.toList):
      case (a, Nil)     => sys.error("zip: Incompatible shapes.")
      case (a, b :: bs) => ((a, b), bs)
    ._1

    def zipL[B](fb: F[B]): F[(A, Option[B])] = fa.mapAccum(fb.toList):
      case (a, Nil)     => ((a, None), Nil)
      case (a, b :: bs) => ((a, Some(b): Option[B]), bs)
    ._1

    def zipR[B](fb: F[B]): F[(Option[A], B)] = fb.mapAccum(fa.toList):
    ...
    def fuse[M[_], N[_], B](f: A => M[B], g: A => N[B])(using
        m: Applicative[M],
        n: Applicative[N]): (M[F[B]], N[F[B]]) = fa
      .traverse[[x] =>> (M[x], N[x]), B](a => (f(a), g(a)))(using m.product(n))
  end extension
...
object Traverse:
  ...
  object Iteration:
    ...
    given iterationFunctor: Functor[Iteration] with
      extension [A](i: Iteration[A])
        def map[B](f: A => B): Iteration[B] = Iteration(f(i.a), b => ???, i.n)
end Traverse
