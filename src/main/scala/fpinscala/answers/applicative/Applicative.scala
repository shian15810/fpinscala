...
trait Applicative[F[_]] extends Functor[F]:
  ...
  def apply[A, B](fab: F[A => B])(fa: F[A]): F[B] = fab.map2(fa)(_(_))
  ...
  extension [A](fa: F[A])
    ...
    def map[B](f: A => B): F[B] = apply(unit(f))(fa)

  def sequence[A](fas: List[F[A]]): F[List[A]] = traverse(fas)(fa => fa)

  def traverse[A, B](as: List[A])(f: A => F[B]): F[List[B]] = as
    .foldRight(unit(List[B]()))((a, acc) => f(a).map2(acc)(_ :: _))
  ...
  def compose[G[_]](G: Applicative[G]): Applicative[[x] =>> F[G[x]]] = new:
    ...
    extension [A](fga: F[G[A]])
      override def map2[B, C](fgb: F[G[B]])(f: (A, B) => C) = self
        .map2(fga)(fgb)(G.map2(_)(_)(f))

  def sequenceMap[K, V](ofv: Map[K, F[V]]): F[Map[K, V]] = ofv
    .foldLeft(unit(Map.empty[K, V])):
      case (acc, (k, fv)) => acc.map2(fv)((m, v) => m + (k -> v))
end Applicative
...
object Applicative:
  ...
  object ZipList:
    ...
    given zipListApplicative: Applicative[ZipList] with
      ...
      extension [A](fa: ZipList[A])
        override def map2[B, C](fb: ZipList[B])(f: (A, B) => C) = fa.zip(fb)
          .map(f.tupled)
  ...
  object WebFormExample:
    ...
    def validBirthdate(birthdate: String): Validated[List[String], LocalDate] =
      try Validated.Valid(LocalDate.parse(birthdate))
        ...
        case _: java.time.format.DateTimeParseException => Validated
    ...
    def validateWebForm(
        name: String,
        birthdate: String,
        phone: String): Validated[List[String], WebForm] = validName(name)
      .map3(validBirthdate(birthdate), validPhone(phone))(WebForm(_, _, _))
  end WebFormExample
  ...
  object WebFormExampleWithNonEmptyList:
    ...
    def validBirthdate(
        birthdate: String): Validated[NonEmptyList[String], LocalDate] =
      try Validated.Valid(LocalDate.parse(birthdate))
      catch
        case _: java.time.format.DateTimeParseException => Validated
            .Invalid(NonEmptyList("Birthdate must be in the form yyyy-MM-dd"))

    def validPhone(
        phoneNumber: String): Validated[NonEmptyList[String], String] =
      if phoneNumber.matches("[0-9]{10}") then Validated.Valid(phoneNumber)
    ...
    def validateWebForm(
      ...
      validName(name)
        .map3(validBirthdate(birthdate), validPhone(phone))(WebForm(_, _, _))
  end WebFormExampleWithNonEmptyList
  ...
  given stateMonad[S]: Monad[State[S, _]] with
    ...
    extension [A](st: State[S, A])
      override def flatMap[B](f: A => State[S, B]): State[S, B] = State
        .flatMap(st)(f)
end Applicative
