...
object IO0:
  ...
  object IO:
    def empty: IO = new:
      ...
      /*  The API of this `IO` type isn't very useful. Not many operations (it
       * is only a monoid), and not many laws to help with reasoning. It is
...
object IO2a:
  ...
  object IO: // Notice that none of these operations DO anything
    ...
    def suspend[A](ioa: => IO[A]): IO[A] = Suspend(() => ioa).flatMap(identity)
  ...
  def printLine(s: String): IO[Unit] = IO.Suspend(() => println(s))

  import IO.monad.*

  val p = printLine("Still going...").forever

  val actions: LazyList[IO[Unit]] = LazyList
    .fill(100000)(printLine("Still going..."))

  val composite: IO[Unit] = actions
    .foldLeft(IO(()))((acc, a) => acc.flatMap(_ => a))
...
object IO2aTests:
  ...
  val g = List.fill(10000)(f)
    .foldLeft(f)((a, b) => x => IO.suspend(a(x).flatMap(b)))
...
object IO2b:
  ...
  object TailRec:
    def apply[A](a: => A): TailRec[A] = suspend(Return(a))

    def suspend[A](a: => TailRec[A]) = Suspend(() => a).flatMap(identity)
...
object IO2bTests:
  ...
  val g: Int => TailRec[Int] = List.fill(10000)(f)
...
object BuiltInTailCalls:
  ...
  val g: Int => TailRec[Int] = List.fill(10000)(f)
    .foldLeft(f)((a, b) => x => tailcall(a(x).flatMap(b)))

  @main
  def runBuiltInTailCalls: Unit =
    val gFortyTwo = g(42)
    println("g(42) = " + gFortyTwo)
...
object IO2c:
  ...
  object Async:
    def apply[A](a: => A): Async[A] = Return(a)

    def suspend[A](aa: => Async[A]): Async[A] = Suspend(Par.delay(aa))
      .flatMap(identity)
...
object IO3:
  ...
  enum Free[F[_], A]:
    ...
    def run(using F: Monad[F]): F[A] = step match
      ...
      case FlatMap(_, _) => sys
          .error("Impossible, since `step` eliminates these cases")

    // return either a `Suspend`, a `Return`, or a right-associated `FlatMap`
    @annotation.tailrec
    final def step: Free[F, A] = this match
      case FlatMap(FlatMap(fx, f), g) => fx.flatMap(x => f(x).flatMap(y => g(y)))
          .step
      case FlatMap(Return(x), f) => f(x).step
      case _                     => this
    ...
    extension [A](fa: Free[Function0, A])
      ...
      def runTrampoline: A = fa match
        ...
        case FlatMap(fx, f) => fx match
            ...
            case FlatMap(fy, g) => fy.flatMap(y => g(y).flatMap(f))
                .runTrampoline
  end Free
  ...
  enum Console[A]:
    ...
    def toState: ConsoleState[A] = this match
      case ReadLine => ConsoleState: bufs =>
          bufs.in match
            case List() => (None, bufs)
    ...
    def toReader: ConsoleReader[A] = this match
      case ReadLine        => ConsoleReader(in => Some(in))
      case PrintLine(line) => ConsoleReader(s => ())
  end Console
  ...
  extension [A](fa: Free[Console, A])
    def toThunk: () => A = fa.runFree([x] => (c: Console[x]) => c.toThunk)

    def toPar: Par[A] = fa.runFree([x] => (c: Console[x]) => c.toPar)
  ...
  extension [A](fa: Free[Console, A])
    def unsafeRunConsole: A = fa.translate([x] => (c: Console[x]) => c.toThunk)
      .runTrampoline
  ...
  extension [A](fa: Free[Console, A])
    def toReader: ConsoleReader[A] = fa
      .runFree([x] => (c: Console[x]) => c.toReader)

    def toState: ConsoleState[A] = fa
      .runFree([x] => (c: Console[x]) => c.toState)
  ...
  def read(
      ...
      fromPosition: Long,
      numBytes: Int): Free[Par, Either[Throwable, Array[Byte]]] = Suspend(
...
object IO4:
  ...
  enum Free[+F[_], A]:
    ...
    @annotation.tailrec
    final def step: Free[F, A] = this match
      case FlatMap(FlatMap(fx, f), g) => fx
          .flatMap(x => f(x).flatMap(y => g(y).covary[F])).step
      case FlatMap(Return(x), f) => f(x).step
      case _                     => this
    ...
    def runFree[G[_]](t: [x] => F[x] => G[x])(using G: Monad[G]): G[A] =
      step match
        case Return(a)  => G.unit(a)
        case Suspend(r) => t(r)
        case FlatMap(Suspend(r), f) => t(r)
            .flatMap[A](a => f(a).covary[F].runFree[G](t))
        case FlatMap(_, _) => sys
            .error("Impossible, since `step` eliminates these cases")
  ...
  object Console:
    def readLn: Free[Console, Option[String]] = Free.Suspend(ReadLine)

    def printLn(line: String): Free[Console, Unit] = Free.Suspend(PrintLine(line))
  ...
  object Files:
    def readLines(file: String): Free[Files, List[String]] = Free
      .Suspend(Files.ReadLines(file))

  def cat(file: String): Free[[x] =>> Files[x] | Console[x], Unit] = Files
    .readLines(file).flatMap: lines =>
      Console.printLn(lines.mkString("\n"))

  extension [A](fa: Free[[x] =>> Files[x] | Console[x], A])
    def toThunk: () => A = fa.runFree(
      [x] =>
...
object IO5:
  ...
  object Console:
    given parConsole: Console[Par] with
      def readLn                = Par.lazyUnit(Try(readLine()).toOption)
      def printLn(line: String) = Par.lazyUnit(println(line))

    given thunkConsole: Console[Function0] with
      def readLn                = () => Try(readLine()).toOption
      def printLn(line: String) = () => println(line)

    given ioConsole: Console[IO] with
      def readLn = Free.Suspend(Par.lazyUnit(Try(readLine()).toOption))
      def printLn(line: String) = Free.Suspend(Par.lazyUnit(println(line)))
  ...
  def cat[F[_]](
      file: String)(using c: Console[F], f: Files[F], m: Monad[F]): F[Unit] = f
    .readLines(file).flatMap: lines =>
