...
object Task:
  extension [A](self: Task[A])
    ...
    def attempt: Task[Try[A]] = IO.monad.map(self):
      case Failure(e) => Success(Failure(e))
      case Success(a) => Success(Success(a))

    def handleErrorWith(h: Throwable => Task[A]): Task[A] = attempt.flatMap:
      case Failure(t) => h(t)
  ...
  given monad: Monad[Task] with
    ...
    extension [A](fa: Task[A])
      def flatMap[B](f: A => Task[B]): Task[B] = IO.monad.flatMap(fa):
