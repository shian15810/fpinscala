...
/**
 * A version of `TailRec` implemented using exceptions. In the implementation of
 * `flatMap`, rather than calling the function, we throw an exception indicating
 * what function we want to call. A central loop repeatedly tries and catches
 * these exceptions to force the computation.
 */
enum Throw[+A]:
  case Done(a: A)
...
object Throw:
  ...
  given monad: Monad[Throw] with
    ...
    extension [A](fa: Throw[A])
      def flatMap[B](f: A => Throw[B]): Throw[B] = fa match
        case Done(a) => f(a)
        case More(thunk) =>
          try thunk().flatMap(f)
          catch
