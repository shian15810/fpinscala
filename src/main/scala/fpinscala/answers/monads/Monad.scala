...
trait Monad[F[_]] extends Functor[F]:
  ...
  extension [A](fa: F[A])
    ...
    def map2[B, C](fb: F[B])(f: (A, B) => C): F[C] = fa
      .flatMap(a => fb.map(b => f(a, b)))

  def sequence[A](fas: List[F[A]]): F[List[A]] = fas
    .foldRight(unit(List[A]()))((fa, acc) => fa.map2(acc)(_ :: _))
  ...
  def filterM[A](as: List[A])(f: A => F[Boolean]): F[List[A]] = as
    .foldRight(unit(List[A]()))(
      (a, acc) => f(a).flatMap(b => if b then unit(a).map2(acc)(_ :: _) else acc))

  extension [A](ffa: F[F[A]]) def join: F[A] = ffa.flatMap(identity)

  extension [A](fa: F[A])
    def flatMapViaJoinAndMap[B](f: A => F[B]): F[B] = fa.map(f).join
...
object Monad:
  ...
  def parserMonad[P[+_]](p: Parsers[P]): Monad[P] = new:
    ...
    extension [A](fa: P[A])
      override def flatMap[B](f: A => P[B]): P[B] = p.flatMap(fa)(f)
  ...
  trait StateMonads[S]:
    ...
    given stateMonad: Monad[StateS] with
      ...
      extension [A](fa: State[S, A])
        override def flatMap[B](f: A => State[S, B]) = State.flatMap(fa)(f)
  ...
  def zipWithIndex[A](as: List[A]): List[(Int, A)] = as
    .foldLeft(F.unit(List[(Int, A)]()))(
      (acc, a) =>
        for
          xs <- acc
          n  <- State.get
...
object Id:
  given idMonad: Monad[Id] with
    def unit[A](a: => A) = Id(a)
    extension [A](fa: Id[A])
      override def flatMap[B](f: A => Id[B]) = fa.flatMap(f)
...
object Reader:
  ...
  given readerMonad[R]: Monad[Reader[R, _]] with
    ...
    extension [A](fa: Reader[R, A])
      override def flatMap[B](f: A => Reader[R, B]) = r => f(fa(r))(r)
