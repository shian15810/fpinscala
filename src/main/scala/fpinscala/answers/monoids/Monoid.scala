...
object Monoid:
  ...
  def monoidLaws[A](m: Monoid[A], gen: Gen[A]): Prop =
    val associativity = Prop.forAll(gen ** gen ** gen):
      case a ** b ** c => m.combine(a, m.combine(b, c)) ==
  ...
  def combineAll[A](as: List[A], m: Monoid[A]): A = as
  ...
  def foldMap[A, B](as: List[A], m: Monoid[B])(f: A => B): B = as
    .foldLeft(m.empty)((b, a) => m.combine(b, f(a)))
  ...
  def parFoldMap[A, B](as: IndexedSeq[A], m: Monoid[B])(f: A => B): Par[B] = Par
  ...
  val orderedMonoid: Monoid[Option[Interval]] = new:
    def combine(oa1: Option[Interval], oa2: Option[Interval]) = (oa1, oa2) match
  ...
  def ordered(ints: IndexedSeq[Int]): Boolean =
    foldMapV(ints, orderedMonoid)(i => Some(Interval(true, i, i))).map(_.ordered)
      .getOrElse(true)
  ...
  val wcMonoid: Monoid[WC] = new Monoid[WC]:
    ...
    def combine(wc1: WC, wc2: WC) = (wc1, wc2) match
      ...
      case (WC.Part(l, w, r), WC.Part(l2, w2, r2)) => WC
  ...
  def count(s: String): Int =
    ...
    def wc(c: Char): WC =
      if c.isWhitespace then WC.Part("", 0, "") else WC.Stub(c.toString)
  ...
  given mapMergeMonoid[K, V](using mv: Monoid[V]): Monoid[Map[K, V]] with
    def combine(a: Map[K, V], b: Map[K, V]) = (a.keySet ++ b.keySet).foldLeft(
      empty): (acc, k) =>
      acc
        .updated(k, mv.combine(a.getOrElse(k, mv.empty), b.getOrElse(k, mv.empty)))
    val empty = Map()
