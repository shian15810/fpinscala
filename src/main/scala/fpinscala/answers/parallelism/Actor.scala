 ...
 * `handler` when it processes the next message, even if the `executor` runs the
 * invocations of `handler` on separate threads. This is achieved because the
 * `Actor` reads a volatile memory location before entering its event loop, and
 * writes to the same location before suspending.
 *
 ...
 * @param onError Exception handler, called if the message handler throws any
 *   `Throwable`.
