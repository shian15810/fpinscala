...
object Prop:
  ...
  def forAll[A](a: Gen[A])(f: A => Boolean): Prop = (max, n, rng) =>
    def go(
        i: Int,
        j: Int,
        l: LazyList[Option[A]],
      ...
      if i == j then Passed(Unfalsified, i)
        ...
        l match
          case Some(h) #:: t =>
            try if f(h) then go(i + 1, j, t, onEnd) else Falsified(h.toString)
  ...
  def apply(f: (TestCases, RNG) => Result): Prop = (_, n, rng) => f(n, rng)
  ...
  def forAll[A](g: Int => Gen[A])(f: A => Boolean): Prop = (max, n, rng) =>
    val casesPerSize = TestCases.fromInt(n.toInt / max + 1)
    val props: List[Prop] = LazyList.from(0).take(max.toInt + 1)
  ...
  val p1 = Prop.forAll(Gen.unit(Par.unit(1)))(
    pi => pi.map(_ + 1).run(executor).get == Par.unit(2).run(executor).get)

  def verify(p: => Boolean): Prop = (_, _, _) => Passed(Proven, 1)
  ...
  def verifyPar(p: Par[Boolean]): Prop = forAllPar(Gen.unit(()))(_ => p)
  ...
  lazy val gpy2: Gen[Par[Int]] = choose(-100, 100).listOfN(choose(0, 20)).map(
    ys => ys.foldLeft(Par.unit(0))((p, y) => Par.fork(p.map2(Par.unit(y))(_ + _))))
...
object Gen:
   ...
   * LazyList(2,3,5)) */
  def cartesian[A](s: LazyList[LazyList[A]]): LazyList[LazyList[A]] = s
    .foldRight(LazyList(LazyList[A]()))(
      (hs, ts) => map2LazyList(hs, ts)(LazyList.cons(_, _)))
   ...
   * any stack space. */
  def sequenceOption[A](o: List[Option[A]]): Option[List[A]] = o
    .foldLeft[Option[List[A]]](Some(List()))((t, h) => map2Option(h, t)(_ :: _))
    .map(_.reverse)
   ...
   * the range. */
  def even(start: Int, stopExclusive: Int): Gen[Int] = choose(
    start,
    if stopExclusive % 2 == 0 then stopExclusive - 1 else stopExclusive)
    .map(n => if n % 2 != 0 then n + 1 else n)

  def odd(start: Int, stopExclusive: Int): Gen[Int] = choose(
    start,
    if stopExclusive % 2 != 0 then stopExclusive - 1 else stopExclusive)
    .map(n => if n % 2 == 0 then n + 1 else n)
  ...
  def listOfN_1[A](n: Int, g: Gen[A]): Gen[List[A]] = List.fill(n)(g)
    .foldRight(unit(List[A]()))((a, b) => a.map2(b)(_ :: _))
  ...
  def union_1[A](g1: Gen[A], g2: Gen[A]): Gen[A] = boolean
    .flatMap(b => if b then g1 else g2)
  ...
  def interleave[A](s1: LazyList[A], s2: LazyList[A]): LazyList[A] = s1
    .map(Some(_)).zipAll(s2.map(Some(_)), None, None)
  ...
  def weighted[A](g1: (Gen[A], Double), g2: (Gen[A], Double)): Gen[A] =
    ...
    Gen(
      State(RNG.double)
        .flatMap(d => if d < g1Threshold then g1._1.sample else g2._1.sample),
      interleave(bools, g1._1.exhaustive, g2._1.exhaustive))

  /* Produce an infinite random lazy list from a `Gen` and a starting `RNG`. */
  def randomLazyList[A](g: Gen[A])(rng: RNG): LazyList[A] = LazyList
  ...
  def interleave[A](
      b: LazyList[Boolean],
      s1: LazyList[A],
      s2: LazyList[A]): LazyList[A] = b.headOption.map: hd =>
    if hd then
      s1 match
  ...
  def stringN(n: Int): Gen[String] = listOfN(n, choose(0, 127))
  ...
  lazy val pint2: Gen[Par[Int]] = choose(-100, 100).listOfN(choose(0, 20)).map(
    l => l.foldLeft(Par.unit(0))((p, i) => Par.fork(p.map2(Par.unit(i))(_ + _))))
...
object Cogen:
  ...
  def takeWhilePropInt =
    forAll(Gen.int.list ** fn(cogenInt, Gen.boolean).unsized)(
      (ys, f) => ys.takeWhile(f).forall(f))

  /* And we can further generalize those properties to be parameterized by types
   * which are not relevant */
  def takeWhileProp[A](ga: Gen[A], ca: Cogen[A]) = forAll(
    ga.list ** fn(ca, Gen.boolean).unsized)((ys, f) => ys.takeWhile(f).forall(f))
end Cogen
