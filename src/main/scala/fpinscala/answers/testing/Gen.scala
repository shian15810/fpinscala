...
object Prop:
  ...
  def forAll[A](g: SGen[A])(f: A => Boolean): Prop = (max, n, rng) =>
    ...
    val props: LazyList[Prop] = LazyList.from(0).take(n.toInt.min(max.toInt) + 1)
      .map(i => forAll(g(i))(f))
    val prop: Prop = props
      .map[Prop](p => (max, n, rng) => p(max, casesPerSize, rng)).toList
      .reduce(_ && _)
    prop(max, n, rng)
  ...
  extension (self: Prop)
    ...
    def ||(that: Prop): Prop = (max, n, rng) =>
      self.tag("or-left")(max, n, rng) match
        ...
        case Falsified(msg, _) => that.tag("or-right")
            .tag(msg.string)(max, n, rng)
        case x => x
  ...
  val gpy2: Gen[Par[Int]] = choose(-100, 100).listOfN(choose(0, 20)).map(
    ys => ys.foldLeft(Par.unit(0))((p, y) => Par.fork(p.map2(Par.unit(y))(_ + _))))

  extension [A](self: List[A])
    def parTraverse[B](f: A => Par[B]): Par[List[B]] = self
      .foldRight(Par.unit(Nil: List[B]))(
        (a, pacc) => Par.fork(f(a).map2(pacc)(_ :: _)))

  val gpy3: Gen[Par[Int]] = choose(-100, 100).listOfN(choose(0, 20))
...
object Gen:
  extension [A](self: Gen[A])
    ...
    def map2[B, C](that: Gen[B])(f: (A, B) => C): Gen[C] = State
      .map2(self)(that)(f)
  ...
  def listOfN[A](n: Int, g: Gen[A]): Gen[List[A]] = State
  ...
  def choose(i: Double, j: Double): Gen[Double] = State(RNG.double)
    .map(d => i + d * (j - i))
  ...
  def union[A](g1: Gen[A], g2: Gen[A]): Gen[A] = boolean
...
object SGen:
  extension [A](self: SGen[A])
    ...
    def **[B](s2: SGen[B]): SGen[(A, B)] = n => Gen.**(apply(n))(s2(n))
