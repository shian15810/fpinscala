...
class GettingStartedSuite extends PropSuite:
  ...
  private val genSortedArray: Gen[Array[Int]] = genList(genShortNumber)
    .map(_.sorted.toArray)

  private val genUnsortedArray: Gen[Array[Int]] =
    for
      n    <- Gen.choose(2, 20)
      list <- Gen.listOfN(n, genShortNumber)
    yield list.zipWithIndex.map:
      case (num, i) => if i % 2 == 0 then num + 100 else num - 100
    .toArray
