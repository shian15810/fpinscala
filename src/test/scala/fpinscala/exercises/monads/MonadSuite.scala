...
class MonadSuite extends PropSuite:
  test("genMonad")(genInt ** genString ** genRNG):
    case n ** s ** rng => assertMonad[Gen](genMonad(rng), n, s)

  test("parMonad")(genInt ** genString):
    case n ** s => assertMonad[Par](parMonad, n, s)

  test("parserMonad")(genInt ** genString):
    case n ** s => assertMonad[Parser[_]](parserMonad(s), n, s)

  test("optionMonad")(genInt ** genString):
    case n ** s => assertMonad[Option[_]](optionMonad, n, s)

  test("lazyListMonad")(genInt ** genString):
    case n ** s => assertMonad[LazyList[_]](lazyListMonad, n, s)

  test("listMonad")(genInt ** genString):
    case n ** s => assertMonad[List[_]](listMonad, n, s)
  ...
  test("Monad.flatMapViaCompose")(genInt ** genRNG):
    case n ** rng =>
      val tm = genMonad(rng)
      import tm.*
      val mappedMonad = monad
        .flatMapViaCompose(pure(n))((i: Int) => pure(i % 2 == 0))
  ...
  test("Monad.flatMapViaJoinAndMap")(genInt ** genRNG):
    case n ** rng =>
      ...
      import tm.*
      val mappedMonad = monad
        .flatMapViaJoinAndMap(pure(n))((i: Int) => pure(i % 2 == 0))
      assertFs(mappedMonad, pure(n % 2 == 0))
  ...
  test("idMonad")(genInt ** genString):
    case n ** s => assertMonad[Id](idMonad, n, s)

  test("readerMonad")(genInt ** genString):
    case n ** s => assertMonad[Reader[Unit, _]](readerMonad, n, s)
...
object MonadSuite extends Assertions:
  private trait TestedMonad[F[_]]:
    val monad: Monad[F]
    def pure[A]: A => F[A]
    def assertFs[A](actual: F[A], expected: F[A]): Unit = Assertions
  ...
  private def genMonad(rng: RNG): TestedMonad[Gen[_]] = new TestedMonad[Gen]:
  ...
  private val parMonad: TestedMonad[Par[_]] = new TestedMonad[Par]:
    val monad: Monad[Par]    = Monad.parMonad
    def pure[A]: A => Par[A] = Par.unit
    override def assertFs[A](actual: Par[A], expected: Par[A]): Unit =
      Assertions
        .assertEquals(actual.run(service).get(), expected.run(service).get())
  ...
  private val optionMonad: TestedMonad[Option[_]] = new TestedMonad[Option]:
  ...
  private val listMonad: TestedMonad[List[_]] = new TestedMonad[List]:
    val monad: Monad[List]    = Monad.listMonad
    def pure[A]: A => List[A] = a => List(a)

  private val idMonad: TestedMonad[Id[_]] = new TestedMonad[Id]:
  ...
  private val readerMonad: TestedMonad[Reader[Unit, _]] =
    new TestedMonad[Reader[Unit, _]]:
      ...
      override def assertFs[A](
          ...
          expected: Reader[Unit, A]): Unit = Assertions
        .assertEquals(actual.run(()), expected.run(()))
end MonadSuite
