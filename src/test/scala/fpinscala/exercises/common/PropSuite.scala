...
trait PropSuite extends FunSuite:
  ...
  private val scalaCheckPropTransform: TestTransform = new TestTransform(
    "FPInScala Prop",
    t =>
      t.withBodyMap(
        _.transformCompat {
  ...
  private def resultToTry(result: Result, test: Test): Try[Unit] = result match
    case Passed(status, n) =>
      println(s"${test.name}: + OK, property ${status.toString.toLowerCase}, ran $n tests.")
      Success(())
    case Falsified(msg) => Try(fail(msg.string)(test.location))
end PropSuite
