...
class CandySuite extends PropSuite:
  ...
  private val genInput: Gen[Input] = Gen.boolean
    .map(b => if b then Coin else Turn)
  private val genInputList: Gen[List[Input]] = genList(genInput)
  ...
  test("Candy: spend some coins")(genLockedMachine ** genPosInt):
    case machine ** myCoins =>
      val wantToSpendAllMyCoins = (0 until myCoins).flatMap(_ => List(Coin, Turn))
        .toList
      val ((coins, candies), machine1): ((Int, Int), Machine) =
