...
class StateSuite extends PropSuite:
  ...
  private val stateA: State[List[String], Option[String]] = State:
    case Nil          => (None, Nil)
    case head :: tail => (Some(head), tail)

  // b - the length of the list, next state - the tail of the list
  private val stateB: State[List[String], Int] = State:
    case Nil          => (0, Nil)
  ...
  private def length(maybeHead: Option[String]): Int = maybeHead.getOrElse("")
    .length
