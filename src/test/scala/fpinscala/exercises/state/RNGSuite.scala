...
class RNGSuite extends PropSuite:
  ...
  test("RNG.nonNegativeInt")(genRNG ** genCounter):
    case rng ** counter => assert(checkRND(rng, counter, nonNegativeInt, _ >= 0))

  // Check that the first n-th numbers of the rng are in the interval [0, 1)
  test("RNG.double")(genRNG ** genCounter):
    case rng ** counter => assert(checkRND(rng, counter, double, isInInterval))
  ...
  test("RNG.double3")(genRNG ** genCounter):
    case rng ** counter =>
      val isCorrect: ((Double, Double, Double)) => Boolean = (d1, d2, d3) =>
        isInInterval(d1) && isInInterval(d2) && isInInterval(d3) && d1 != d2 &&
          d1 != d3 && d2 != d3
  ...
  test("RNG.int")(genRNG ** genCounter):
    case rng ** counter => assert(checkRND(rng, counter, int, _ => true))
  ...
  test("RNG.flatMap")(genRNG ** genCounter ** genSmallPosNum):
    case rng ** counter ** limit =>
      ...
      assert(checkRNGNonNegativeLessThan(
        rng,
        counter,
  ...
  test("RNG.mapViaFlatMap")(genRNG ** genCounter):
    case rng ** counter => assert(checkRND(
        rng,
        counter,
  ...
  private def checkRNGUnit[A](
      rng: RNG,
      counter: Int,
      constantValue: A): Boolean =
    @tailrec
    def loop(rng: RNG, counter: Int): Boolean = counter < 0 || {
      val (value, rng2) = RNG.unit(constantValue)(rng)
      if constantValue == value then loop(rng2, counter - 1)
      else
        fail(s"RNG.unit must always return a constant value ('$constantValue').")
    }
    loop(rng, counter)
