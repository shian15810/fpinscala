 ...
 *
 * @see [[fpinscala.answers.parsing.Reference]]
 */
object UnitTestParser extends Parsers[UnitTestParser.Parser]:
  ...
  def firstNonmatchingIndex(s1: String, s2: String, offset: Int): Int =
    ...
    while i + offset < s1.length && i < s2.length do
      ...
      i += 1
    if s1.length - offset >= s2.length then -1 else s1.length - offset

  def string(w: String): Parser[String] = l =>
    val i = firstNonmatchingIndex(l.input, w, l.offset)
    if i == -1 then Success(w, w.length)
    else Failure(l.advanceBy(i).toError(s"'$w'"), i != 0)

  def regex(r: Regex): Parser[String] = l =>
  ...
  extension [A](p: Parser[A])
    ...
    def run(s: String): Either[ParseError, A] = p(Location(s)).extract

    def or(p2: => Parser[A]): Parser[A] = l =>
      p(l) match
    ...
    def flatMap[B](f: A => Parser[B]): Parser[B] = l =>
      p(l) match
        case Success(a, n) => f(a)(l.advanceBy(n)).addCommit(n != 0)
            .advanceSuccess(n)
        case f @ Failure(_, _) => f

    def slice: Parser[String] = l =>
      p(l) match
        case Success(a, n)     => Success(l.slice(n), n)
        case f @ Failure(_, _) => f

    def scope(msg: String): Parser[A] = l => p(l).mapError(_.push(l, msg))

    def label(msg: String): Parser[A] = l => p(l).mapError(_.label(msg))
  end extension
end UnitTestParser
