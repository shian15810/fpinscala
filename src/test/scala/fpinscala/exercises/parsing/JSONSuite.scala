...
class JSONSuite extends PropSuite:
  ...
  test("JSON.JObject, 1")(Gen.unit(())): _ =>
    assertEquals(
      ...
      Right(JObject(Map(
        ...
        "Related companies" -> JArray(IndexedSeq(
          JString("HPQ"),
          JString("IBM"),
          JString("YHOO"),
          JString("DELL"),
      ...
      Right(JObject(Map(
        ...
        "Parts" -> JObject(Map(
          "Part 1" -> JObject(Map(
            ...
            "Content" -> JArray(IndexedSeq(JObject(Map(
              "Chapter 1" -> JObject(Map(
                "Title" -> JString("What is functional programming?"),
                "Content" -> JArray(IndexedSeq(
  ...
  private val genMalformedJSONs: Gen[String] = IndexedSeq(
    """
    "Company name"
    """,
    """
